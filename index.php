...
<?php 
  if(isset($_POST["btn-login"]))
  {
    ...
    if(!isset($password_err) && !isset($username_err)){
		...
		if($userexists && password_verify($pass, $hashed_password)){
			error_log($pass);
			$name = $row["name"];
			$id = $row["usr_id"];
			session_start();
...
<div class="container">
	...
	<div class="padding-right">
		...
		<ul class="job-list full">
			<?php 
           $sql=mysqli_query($con,"select * from posts join registration where posts.usr_id_p=registration.usr_id order by posts.status_time desc");
